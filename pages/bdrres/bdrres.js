...
Page({
  ...
  fsub:function(e){
    ...
    that.data.hidden = false;
      wx.request({
      url: app.baseUrl.Url+'repairOrder/update/problemFeedback',
      method: 'PUT',
      header: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'token': wx.getStorageSync("retoken")
      },
        data: { 'orderNo': wx.getStorageSync("orderNo"), 'result': '维保失败', 'cause': that.data.ceshi},
      success: function (res) {
  ...
  sub:function(e){
    ...
    wx.request({
      ...
      data: { 'orderNo': wx.getStorageSync("orderNo"), 'result': '维保成功', 'note': that.data.remark, 'imager': that.data.imgsurl},
      success: function (res) {
        that.data.hidden = true;
        util.tusi('上报成功', that.data.setInter = setInterval(function () {
          wx.navigateBack();
